  ...
  const validateStep = () => {
    switch (currentStep) {
      ...
      case 2:
        ...
        break;
      case 3:
        // Practices & Creativity - no required fields
        break;
      case 4:
        // Agreements
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (isDualRole && isExistingUser) {
        const {
          data: existingSub
        } = await supabase.from('subscriptions').select('*').eq('user_id', currentUser.id).eq('status', 'active').single();
    ...
    return <div className="min-h-screen bg-background">
        ...
        <main className="pt-16 sm:pt-20 pb-24">
          <div className="flex items-center justify-center p-4 pt-8">
            {showWelcome && <Card className="w-full max-w-3xl">
            ...
            {showAuth && !isExistingUser && <Card className="w-full max-w-md">
              <CardHeader className="text-center">
                <CardTitle className="text-2xl font-bold">
                  {isLogin ? "Sign In" : "Create Your Account"}
                </CardTitle>
                <CardDescription>
                  {isLogin ? "Already have a shopper account? Sign in to continue." : "Create a new account to get started."}
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                {!isLogin && <div className="space-y-2">
                    <Label htmlFor="fullName">Full Name</Label>
                    <Input id="fullName" type="text" placeholder="John Doe" value={authData.fullName} onChange={e => setAuthData({
                  ...authData,
                ...
                <div className="space-y-2">
                  ...
                  <Input id="email" type="email" placeholder="your@email.com" value={authData.email} onChange={e => setAuthData({
                  ...authData,
                ...
                <div className="text-center">
                  <Button variant="link" onClick={() => setIsLogin(!isLogin)} className="text-sm">
                    {isLogin ? "Don't have an account? Sign up" : "Already have a shopper account? Sign in"}
                  </Button>
                </div>
              </CardContent>
            </Card>}
            ...
            <CardContent className="space-y-6">
              ...
              {currentStep === 0 && <>
                  {isExistingUser && <div className="p-4 bg-primary/10 rounded-lg mb-4">
                      ...
                      <div className="space-y-2">
                        ...
                        <Input id="updateName" type="text" placeholder={authData.fullName || "Current name"} value={authData.fullName} onChange={e => setAuthData({
                    ...authData,
                    fullName: e.target.value
                  })} />
                      </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      ...
                      <Input id="city" value={formData.city} onChange={e => updateField('city', e.target.value)} placeholder="Portland" />
                    </div>
                    <div>
                      <Label htmlFor="state">State/Region *</Label>
                      <Input id="state" value={formData.state_region} onChange={e => updateField('state_region', e.target.value)} placeholder="Oregon" />
                    </div>
                  </div>

                  <div>
                    <Label htmlFor="country">Country *</Label>
                    <Input id="country" value={formData.country} onChange={e => updateField('country', e.target.value)} placeholder="United States" />
                  </div>

                  <div>
                    <Label htmlFor="phone">Phone Number (Optional)</Label>
                    <Input id="phone" type="tel" value={formData.phone_number} onChange={e => updateField('phone_number', e.target.value)} placeholder="+1 (555) 123-4567" />
                  </div>
                </>}
              ...
              {currentStep === 1 && <>
                  <div>
                    ...
                    <Textarea id="description" value={formData.business_description} onChange={e => updateField('business_description', e.target.value)} placeholder="Tell us about your business..." rows={4} />
                  </div>
                  ...
                  {(formData.business_type === "diverse" || formData.business_type === "other") && <div>
                      <Label htmlFor="businessTypeOther">Please Specify</Label>
                      <Input id="businessTypeOther" value={formData.business_type_other} onChange={e => updateField('business_type_other', e.target.value)} />
                    </div>}
                </>}
            ...
            {currentStep === 2 && <>
              ...
              {currentStep === 4 && <>
                  <div>
                    ...
                    <Textarea id="additional" value={formData.additional_info} onChange={e => updateField('additional_info', e.target.value)} placeholder="Share any additional information..." rows={4} />
                  </div>
              ...
              <div className="flex justify-between pt-6 border-t">
                ...
                {currentStep < totalSteps - 1 ? <Button onClick={handleNext}>
                    Next
