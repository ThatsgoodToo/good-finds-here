...
const VendorDashboard = () => {
  ...
  useEffect(() => {
    const loadMetrics = async () => {
      ...
      const {
        data: vendorProfile
      } = await supabase.from("vendor_profiles").select("profile_views, clicks_to_website").eq("user_id", user.id).maybeSingle();
      const {
        data: coupons
      } = await supabase.from("coupons").select("id").eq("vendor_id", user.id).eq("active_status", true);
      const {
        data: followers
      } = await supabase.from("followers").select("id").eq("vendor_id", user.id);
      setMetrics({
  ...
  useEffect(() => {
    const loadFollowers = async () => {
      ...
      const {
        data: followers
      } = await supabase.from("followers").select(`
          shopper_id,
          created_at,
          profiles!followers_shopper_id_fkey(display_name, avatar_url, profile_picture_url)
        `).eq("vendor_id", user.id).order("created_at", {
        ascending: false
      }).limit(10);
      if (followers) {
        setRecentVisitors(followers.map((f: any) => ({
          id: f.shopper_id,
  ...
  useEffect(() => {
    const loadListings = async () => {
      ...
      const {
        data
      } = await supabase.from("listings").select("*").eq("vendor_id", user.id);
      if (data) {
  ...
  return <div className="min-h-screen bg-background">
      ...
      <main className="pt-16 sm:pt-20 pb-12">
        <VendorHeader vendorName={vendorName} vendorImage={vendorImage} location={location} externalUrl={externalUrl} description={vendorDescription} vendorUserId={user?.id || ""} onUploadImage={handleUploadImage} onUpdateLocation={handleUpdateLocation} onUpdateExternalUrl={handleUpdateExternalUrl} onUpdateDescription={handleUpdateDescription} />
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
            <TabsList className="grid w-full grid-cols-2 sm:grid-cols-4 mb-8">
              ...
              <TabsTrigger value="coupons" className="gap-2" data-tour="vendor-offers">
                ...
                {metrics.activeOffers === 0 && <Badge variant="destructive" className="ml-1">!</Badge>}
              </TabsTrigger>
            ...
            <TabsContent value="overview" className="space-y-6">
              ...
              {metrics.activeOffers === 0 && <Alert className="bg-yellow-50 dark:bg-yellow-950/20 border-yellow-200 dark:border-yellow-800">
                  ...
                  <AlertDescription className="text-yellow-700 dark:text-yellow-400 flex items-center justify-between">
                    ...
                    <Button variant="outline" size="sm" className="ml-4 shrink-0 border-yellow-300 hover:bg-yellow-100 dark:border-yellow-700 dark:hover:bg-yellow-900/30" onClick={() => setShowCouponDialog(true)}>
                      Create Coupon
                    </Button>
                  </AlertDescription>
                </Alert>}
              ...
              {/* Metrics */}
              <MetricsRow clicks={metrics.clicks} sales={metrics.sales} activeOffers={metrics.activeOffers} followers={metrics.followers} onMetricClick={handleMetricClick} data-tour="vendor-metrics" />

              {/* Filters */}
              <VendorFilters mainCategories={mainCategories} subcategories={subcategories} onEditSubcategories={handleEditSubcategories} />
            </TabsContent>
            ...
            <TabsContent value="coupons" className="space-y-6">
              <Card>
                ...
                <CardContent>
                  <CouponList refresh={refreshCoupons} onRefreshComplete={() => setRefreshCoupons(false)} onCreateClick={() => setShowCouponDialog(true)} />
                </CardContent>
              </Card>
            </TabsContent>
              ...
              <Card>
                ...
                <CardContent>
                  <div className="space-y-3">
                    {recentVisitors.map(visitor => <div key={visitor.id} className="flex items-center justify-between p-3 border rounded-lg hover:bg-accent/50 transition-colors">
                        <div className="flex items-center gap-3">
                          <img src={visitor.image} alt={visitor.name} className="w-10 h-10 rounded-full object-cover" />
                          <div>
                            <p className="font-medium">{visitor.name}</p>
                            <p className="text-xs text-muted-foreground">Saved {visitor.lastVisit}</p>
                          </div>
                        </div>
                        <Button size="sm" variant="outline" onClick={() => handleSendOffer(visitor.id)}>
                          Send Offer
                        </Button>
                      </div>)}
                  </div>
      ...
      {selectedMetric && <MetricsChart metric={selectedMetric} onClose={() => setSelectedMetric(null)} />}
      ...
      {/* Share Coupon Dialog */}
      {selectedShopper && <ShareCouponDialog open={shareDialogOpen} onOpenChange={setShareDialogOpen} shopperId={selectedShopper.id} shopperName={selectedShopper.name} />}
